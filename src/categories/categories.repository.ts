...
export class CategoriesRepository {
  ...
  async addCategories() {
    const categoriesWithProducts = await this.categoriesRepository.find({
      relations: { products: { category: true } },
      where: { products: MoreThan(0) },
    });

    const categoriesToNotDelete = categoriesWithProducts.map(
      (category) => category.id,
    );

    await this.categoriesRepository.delete({
      id: Not(In(categoriesToNotDelete)),
    });
    ...
    return 'Categorías cargadas exitosamente.';
  }
  ...
  async deleteCategory(id: string) {
    ...
    if (categoryToDelete.products.length !== 0) {
      throw new ForbiddenException(
        `Categoría ID: ${id} tiene al menos un producto asociado. No puede ser eliminada.`,
      );
    }
