...
export class UsersRepository {
  ...
  async createAdmin(){
    ...
    .values({
      ...
      password: "$2b$10$SEDsfd18gVw0M4YmFBBC0uHsLrHZgvLXkmmMP0F81zVBRYjCcGN0y",
      isAdmin: true
    })
    .orUpdate(['name', 'password', 'phone', 'isAdmin'], ['email'])
    .execute()
  }
  ...
  async deleteUser(userIdToDelete: string, requestedUserId: string) {
    ...
    if (
      (requestingUser.id === userToDelete.id) || (requestingUser.isAdmin && !userToDelete.isAdmin) 
    ) {
